...
from SELFIES_vocab import get_vocab,selfies_to_ids,ids_list_to_smiles
...
def load_data(csv_file):
    ...
    full_vocab,vocab_to_idx,idx_to_vocab = get_vocab(list(smiles_list))
    ...
    for item in smiles_list:
        ...
        smiles_ids = selfies_to_ids(vocab_to_idx,temp)
    ...
    for s, other_item in zip(smiles_list, normalized_other):
        ...
        if graph_data is not None:
            temp = sf.encoder(s)
            smiles_ids = selfies_to_ids(vocab_to_idx,temp,True,max_smiles_ids_len)
    ...
    for i in range(len(graph_data_list)):
        node_features, edge_features, edge_src, edge_dst,selfies= graph_data_list[i][0],graph_data_list[i][1],graph_data_list[i][2],graph_data_list[i][3],graph_data_list[i][4]
        nf_num = node_features.shape[0]
        ef_num = len(edge_features)
        selfies_list.append(selfies)
        if nf_num < max_nodes:
            nf =  np.pad(node_features, ((0, max_nodes - nf_num), (0, 0)), mode='constant', constant_values=0)
            nf_list.append(nf)
        else:
            nf_list.append(node_features)
        if(ef_num<max_bond_num):
            ef = np.pad(edge_features, ((0, max_bond_num - ef_num), (0, 0)), mode='constant', constant_values=0)
            ef_list.append(ef)
            es = np.pad(edge_src, ((0, max_bond_num - ef_num)), mode='constant', constant_values=0)
            es_list.append(es)
            ed = np.pad(edge_dst, ((0, max_bond_num - ef_num)), mode='constant', constant_values=0)
            ed_list.append(ed)
        else:
            ef_list.append(edge_features)
            es_list.append(edge_src)
            ed_list.append(edge_dst)
            
            
    dataset = tf.data.Dataset.from_tensor_slices((tf.convert_to_tensor(nf_list,dtype=tf.float32,name='node_features'),
                                                  tf.convert_to_tensor(ef_list,dtype=tf.float32,name='edge_features'), 
                                                  tf.convert_to_tensor(es_list,dtype=tf.int32,name='edge_src'), 
                                                  tf.convert_to_tensor(ed_list,dtype=tf.int32,name='edge_dst'),
                                                  tf.convert_to_tensor(other_data_list,dtype=tf.float32,name='prop'),
                                                  tf.convert_to_tensor(selfies_list,dtype=tf.int32,name='smiles'),
                                                  ))
    
    # node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num,vocab_size
    ...
    res_prop={
        ...
        "max_atom_type_num":max_atom_type_num,
        "max_bond_type_num":max_bond_type_num,
        "edge_len_num":edge_len_num,
        "max_smiles_ids_len":max_smiles_ids_len,
    }
    
    return dataset,res_prop
...
class VAELossCalculator:
    ...
    def compute_loss(self, targets, predictions, z_mean, z_log_var,physical_score):
        """
        ...
        physical_score = tf.cast(physical_score,dtype=tf.float32)
        physical_loss = tf.constant(physical_score, dtype=tf.float32)
        physical_loss = tf.stop_gradient(physical_loss)  # 关键操作
        # 3. 组合总损失
        total_loss = recon_loss + self.kl_weight * kl_loss+physical_loss
        
        return total_loss, recon_loss, kl_loss,physical_loss
...
class GAN(keras.Model):
    ...
    def physical_score(self,predictions,multiple=10):
        ...
        global VOCAB_TO_IDX
        smiles_list = ids_list_to_smiles(predicted_ids_list,IDX_TO_VOCAB)
        # smiles_list = VOCAB_TO_IDX.ids_list_to_smiles(predicted_ids_list)
        for smiles in smiles_list:
            ...
            if(mol is None):
                ...
                try:
                    ...
                    fingerprint_array = fps
                except Exception as e:
                    fingerprint_array = None
                if(fingerprint_array is not None and isinstance(fingerprint_array,  DataStructs.cDataStructs.ExplicitBitVect) and isinstance(fingerprint_array,np.ndarray) is False):
                    fp_list.append(fingerprint_array)
                # 检查当前生成的分子是否重复
                if(isinstance(fingerprint_array,np.ndarray)):
                    duplicate_penalty_score = 1
                else:
                    duplicate_penalty_score = self.is_duplicate(fingerprint_array)
                # ex_score_g = EXPhysicalScore(mol)
                # ex_score = ex_score_g.cal_score()
                ex_score = p_score(mol=mol)
                res = res+duplicate_penalty_score+ex_score
        # for i in self.fingerprint_list:
        #     if(isinstance(i,  DataStructs.cDataStructs.ExplicitBitVect) is False):
        #         print(i)
        #         if(isinstance(i,np.ndarray)):
        #             print(i)
        self.fingerprint_list.update(fp_list)
        return (res/self.batch_size)*multiple
    def is_duplicate(self,current_fingerprint, threshold=0.95):
        if(len(self.fingerprint_list)==0):
            return 0
        max_score, max_index = self.find_max_similarity(current_fingerprint,self.fingerprint_list)
        if(max_score>0.95):
            return 1
        else:
            return 0
        
    def find_max_similarity(self,query_fp, fp_list):
        """
        计算并返回最大相似度及其索引
        :param query_fp: 目标指纹(ExplicitBitVect)
        :param fp_list: 待比对指纹列表
        :return: (最大相似度, 索引)
        """
        try:
            if len(fp_list) == 0:
                raise ValueError("指纹列表不能为空")
            
            if(isinstance(query_fp,np.ndarray)):
                print(query_fp)
            
            # 批量计算相似度
            similarity_scores = DataStructs.BulkTanimotoSimilarity(query_fp, fp_list)
            ...
            return 0,0
    def is_duplicate2(self,current_fingerprint, threshold=0.98):
        """
        判断当前分子是否与历史分子重复,
    ...
    def call(self, inputs, training=True):
        # 输出邻接矩阵和节点特征
        z_mean, log_var = self.encoder(inputs,training=training)
        ...
        predictions = self.decoder((decoder_input, z),training=training)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            physical_score = self.physical_score(predictions)
                    # 计算损失
            total_loss, recon_loss, kl_loss,physical_loss = self.loss_calculator.compute_loss(
                targets, predictions, z_mean, log_var,physical_score
            )
        ...
        return{"loss":loss,"recon_loss":recon_loss,"kl_loss":kl_loss,"physical_score":physical_loss}
...
def p_score(mol):
    ...
    try:
        ...
        if any(not c for c in chiral_centers):
            res = res+1
        aromatic_system = validate_aromatic_system(mol=mol)
        if(aromatic_system is False):
            res = res+1
        aromatic_topology = check_aromatic_topology(mol=mol)
        if(aromatic_topology is False):
            res = res+1
        return res
    except Exception as e:
        return 3
...
def validate_aromatic_system(mol):
    """2025版芳香系统验证方法"""
    try:
        # 创建分子拷贝防止原数据修改
        ...
        # 键级合理性检查
        bond_types = set(b.GetBondType() for b in test_mol.GetBonds())
        if Chem.BondType.AROMATIC in bond_types:
            return False, "AROMATIC标记残留"
            
        return True, "Kekul化成功"
        
    except Chem.KekulizeException as e:
        return False, f"Kekul化失败: {str(e)}"
    
def check_aromatic_topology(mol):
    """基于环检测的深度验证"""
    ...
    for bond in mol.GetBonds():
        if bond.GetIsAromatic():
            if bond.GetBondType() not in {Chem.BondType.SINGLE, Chem.BondType.DOUBLE}:
                return False, f"异常芳香键类型: {bond.GetBondType()}"
                
    return len(aromatic_atoms) > 0, "芳香系统拓扑有效"
...
class TransformerDecoderBlock(layers.Layer):
    ...
    def call(self, x, z, training, look_ahead_mask):
        ...
        attn2 = self.mha2(
            query=out1,  # 使用解码器的输出作为query
            key=z_proj,   # 使用编码器输出的投影作为key
            value=z_proj  # 使用编码器输出的投影作为value
        )
...
class SMILESDecoder(keras.Model):
    ...
    def __init__(self, hp,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,vocab_size):
        super(SMILESDecoder, self).__init__()
        # 原有参数初始化...
        
        # 增加多样性机制
        self.temperature = tf.Variable(1.3, trainable=False)  # 生成温度
        ...
        self.final_layer = layers.Dense(self.vocab_size,kernel_regularizer=keras.regularizers.l2(1e-4))
        global VOCAB_TO_IDX

        vocab = VOCAB_TO_IDX
        valence_rules = {'C':4, 'N':3, 'O':2, 'S':2}
    ...
    def call(self, inputs, training=True):
        ...
        # combined_mask = tf.cast(combined_mask > 0, tf.float32)[..., tf.newaxis]
        ...
        for i in range(len(self.dec_layers)):
            x = self.dec_layers[i](x, z, training=training,  look_ahead_mask=combined_mask)
        # x = self.dec_layers(x, z, training=training, look_ahead_mask=combined_mask)
        logits = self.final_layer(x)
        output = logits / self.temperature
        if training:
            output = self.noise_layer(output, training=training)
        # res = self.valence_layer((output,data))
        return output
...
class EncoderModel(keras.Model):
    def __init__(self, hp,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num):
        ...
        self.z_mean = layers.Dense(256, dtype="float32", kernel_regularizer=keras.regularizers.l2(0.01), name="z_mean")
        self.log_var = layers.Dense(256, dtype="float32", kernel_regularizer=keras.regularizers.l2(0.01), name="log_var")
...
class HyperGAN(kt.HyperModel):
    ...
    def __init__(self,config,props):
        ...
        self.nf_shape =props.get("node_features_shape")
        self.ef_shape = props.get("edge_features_shape")
        # self.input_dim3_shape = input_dim3_shape
        self.max_atom_num =props.get("max_atom_num") 
        self.max_atom_type_num = props.get("max_atom_type_num") 
        self.max_bond_type_num =props.get("max_bond_type_num")
        self.edge_len_num = props.get("edge_len_num")
        self.max_smiles_ids_len = props.get("max_smiles_ids_len")
        self.vocab_size = props.get("vocab_size")
    ...
    def make_decoder_model(self, hp):
        ...
        decoder = SMILESDecoder(hp,self.config,self.nf_shape, self.ef_shape,self.max_atom_num,self.max_atom_type_num,self.max_bond_type_num,self.edge_len_num,self.max_smiles_ids_len,self.vocab_size)
...
def main():
    ...
    dataset,props = load_data(csv_file)
    ...
    tuner = kt.BayesianOptimization(
        hypermodel=HyperGAN(search_space,props),
