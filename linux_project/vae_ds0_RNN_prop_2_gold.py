...
class PositionalEncoding(layers.Layer):
    ...
    def __init__(self, d_model, max_len=512):
        ...
        sines = np.sin(angle_rads[:, 0::2])  # 偶数索引使用sin
        cosines = np.cos(angle_rads[:, 1::2]) # 奇数索引使用cos
...
class SyntaxAwareDecoder(layers.Layer):
    """语法感知解码层"""
    def __init__(self, vocab, max_length,vocab_reverse):
        super().__init__()
        self.vocab = vocab
        self.max_length = max_length
        self.vocab_reverse = vocab_reverse
        self._build_allowed_transitions()

    def _build_allowed_transitions(self):
        """构建符合SMILES语法的转移规则"""
        # 定义字符类别
        atoms = ['C','O','N','S','P','F','Cl','Br','I','*','c','n','s','o','p']  # 原子符号（含芳香原子）
        bonds = ['=', '#', '-', ':']  # 化学键（包含芳香键':'）
        digits = [str(i) for i in range(10)]  # 环编号
        branch_open = ['(']
        branch_close = [')']
        ring_open = ['%']  # 两位环编号前缀
        bracket_open = ['[']
        ...
        # 初始化规则字典
        self.allowed = defaultdict(lambda: {'allowed': []})
        # 显式初始化特殊符号
        self.allowed['<sos>'] = {'allowed': []}
        self.allowed['<eos>'] = {'allowed': []}
        # ---- 特殊符号规则 ----
        self.allowed['<sos>']['allowed'] = atoms + bracket_open + branch_open + digits
        self.allowed['<eos>']['allowed'] = []
        
        # ---- 原子符号规则 ----
        atom_allowed = atoms + bonds + digits + branch_open + branch_close + bracket_close + ['<eos>']
        for a in atoms:
            if(a not in self.allowed):
                self.allowed[a]= {'allowed': []}
            self.allowed[a]['allowed'] = atom_allowed.copy()
            # 芳香原子特殊规则（不能直接连接非芳香键）
            if a.islower():
                self.allowed[a]['allowed'] = [x for x in self.allowed[a]['allowed'] if x not in bonds[:3]]

        # ---- 化学键规则 ----
        bond_allowed = atoms + branch_open + bracket_open
        for b in bonds:
            if(b not in self.allowed):
                self.allowed[b]= {'allowed': []}
            self.allowed[b]['allowed'] = bond_allowed.copy()
            # 双键/三键不能连接芳香原子
            if b in ['=', '#']:
                self.allowed[b]['allowed'] = [x for x in bond_allowed if not x.islower()]

        # ---- 括号规则 ----
        self.allowed['(']= {'allowed': []}
        self.allowed['(']['allowed'] = atoms + bonds + digits + branch_open + bracket_open
        self.allowed[')']= {'allowed': []}
        self.allowed[')']['allowed'] = bonds + digits + branch_close + bracket_close + ['<eos>']

        # ---- 环编号规则 ----
        digit_allowed = atoms + bonds + branch_open + branch_close + digits + ['<eos>']
        for d in digits:
            if(d not in self.allowed):
                self.allowed[d]= {'allowed': []}
            self.allowed[d]['allowed'] = digit_allowed.copy()
            # 环编号不能直接跟随非数字字符（两位数需要%前缀）
            if d == '%':
                self.allowed[d]['allowed'] = digits
            else:
                self.allowed[d]['allowed'] = [x for x in digit_allowed if x != '%']

        # ---- 方括号规则（复杂原子表示）----
        self.allowed['[']= {'allowed': []}
        self.allowed[']']= {'allowed': []}
        self.allowed['[']['allowed'] = digits + atoms + hydrogens + ['[', ']']
        self.allowed[']']['allowed'] = bonds + digits + branch_close + ['<eos>']
        
        # 方括号内电荷规则
        for c in charges:
            if(c not in self.allowed):
                self.allowed[c]= {'allowed': []}
            self.allowed[c]['allowed'] = digits + [']']
            # 允许+2, -3等表示
            self.allowed[c+'1'] = {'allowed': [']']}
            self.allowed[c+'2'] = {'allowed': [']']}
            self.allowed[c+'3'] = {'allowed': [']']}

        # ---- 氢原子规则 ----
        self.allowed['H']= {'allowed': []}
        self.allowed['H']['allowed'] = digits + [']'] + charges
        
        self.allowed['%']= {'allowed': []}
        # ---- 两位环编号规则 ----
        self.allowed['%']['allowed'] = digits
        
        # ---- 芳香键规则 ----
        self.allowed[':']= {'allowed': []}
        self.allowed[':']['allowed'] = [a for a in atoms if a.islower()] + branch_open + bracket_open
        ...
        # 连续两个化学键
        for b in bonds:
            self.allowed[b]['allowed'] = [x for x in self.allowed[b]['allowed'] if x not in bonds]
        
        # 方括号内语法增强
        bracket_inside = atoms + hydrogens + digits + charges + [']', '[', ':']
        for sym in bracket_inside:
            if sym in self.allowed:
                self.allowed[sym]['allowed'] += [']']
        
    def _compute_mask(self, last_tokens):
        # last_tokens: numpy 数组，形状 (batch_size,)
        batch_size = last_tokens.shape[0]
        mask = np.zeros((batch_size, len(self.vocab)), dtype=np.float32)
        for i in range(batch_size):
            token_index = int(last_tokens[i])
            token = self.vocab_reverse.get(token_index, None)
            allowed_tokens = self.allowed.get(token, None)
            if allowed_tokens is not None:
                allowed_indices = [self.vocab[t] for t in allowed_tokens if t in self.vocab]
                for j in range(len(self.vocab)):
                    if j not in allowed_indices:
                        mask[i, j] = -1e9
        return mask
        
    def _get_mask(self, sequence):
        # 取出当前序列中最后一个 token： shape 为 (batch_size,)
        last_tokens = sequence[:, -1]
        mask = tf.py_function(self._compute_mask, [last_tokens], tf.float32)
        mask.set_shape([None, len(self.vocab)])
        return mask
    ...
    #     mask = tf.zeros((batch_size, seq_len, len(self.vocab)))
    ...
    #             temp = sequence_numpy[b, t]
    #             token = self.vocab_reverse[temp]
    #             if token == '(': stack.append('(')
    #             if token == ')':
    #                 if len(stack) == 0:
    #                     mask[b, t, self.vocab['(']] = -1e9  # 不允许继续闭合
    #                 else:
    #                     stack.pop()
    #             # 根据当前状态设置允许的token
    #             # 需要扩展完整的语法规则...
    ...
    def call(self, logits, current_sequence):
        # mask = self._get_mask(current_sequence)
        # return logits + mask
        # logits: shape (batch_size, seq_len, vocab_size)
        last_logits = logits[:, -1, :]  # 最后一步的 logits
        mask = self._get_mask(current_sequence)  # 形状 (batch_size, vocab_size)
        constrained_last_logits = last_logits + mask
        # 用调整后的最后一步替换原 logits
        logits_constrained = tf.concat([logits[:, :-1, :],
                                        tf.expand_dims(constrained_last_logits, axis=1)],
                                       axis=1)
        return logits_constrained
        

class TransformerDecoder(keras.Model):
    """完整的Transformer解码器"""
    def __init__(self, vocab_size, latent_dim, d_model=256, 
                 num_layers=4, num_heads=8, dff=512, max_length=128, 
                 dropout=0.1, vocab=None):
        super().__init__()
        self.d_model = d_model
        ...
        # 解码器堆叠
        self.dec_layers = [TransformerDecoderBlock(d_model, num_heads, dff, dropout)
                           for _ in range(num_layers)]
        
        # 潜在变量投影
        self.z_proj = layers.Dense(d_model)
        
        # 输出层
        self.fc = layers.Dense(vocab_size)
        
        # 语法约束层
        self.syntax_layer = SyntaxAwareDecoder(vocab, max_length,self.vocab_reverse)
    ...
    def call(self, inputs, z, training=True):
        ...
        z_proj = tf.expand_dims(z_proj, 1)  # 增加序列维度
        
        # 通过解码器层
        for dec_layer in self.dec_layers:
            # (self, x, z, training, look_ahead_mask):
            x = dec_layer(x=x, z=z_proj, training=training, look_ahead_mask=look_ahead_mask)
        
        # 最终投影
        logits = self.fc(x)
        
        # 应用语法约束
        constrained_logits = self.syntax_layer(logits, inputs)
...
class HyperGAN(kt.HyperModel):
    ...
    def make_decoder_model(self, hp):
        ...
        decoder = TransformerDecoder(len(vocab),128,vocab=vocab)
