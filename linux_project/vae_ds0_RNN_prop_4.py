...
# 为了解决环的问题使用掩码，但效果不是很好，尤其环约束还没实现完全
...
# class ChemicalConstraintLayer(layers.Layer):
#     def __init__(self, vocab, rule_functions, **kwargs):
#         super().__init__(**kwargs)
#         self.vocab = vocab
#         self.rule_functions = rule_functions
#         self.token_map = {v: k for k, v in vocab.items()}
...
#             rule_mask = rule_fn(x, self.vocab, logits.shape[-1])
#             mask = tf.logical_and(mask, rule_mask)
        
#         return tf.where(mask, logits, -1e9)
...
class ChemicalConstraintLayer(layers.Layer):
    ...
    def call(self, inputs):
        x, logits = inputs
        mask = tf.ones_like(logits, dtype=tf.bool)  # 初始形状 (batch, seq, vocab)
        
        for rule_fn in self.rule_functions:
            # 确保每个规则返回形状 (batch, seq, vocab)
            rule_mask = rule_fn(x, self.vocab, logits.shape[-1])
            
            # 添加维度验证
            tf.debugging.assert_shapes([
                (rule_mask, ('batch', 'seq', 'vocab')),
...
# def parentheses_rule(x, vocab, vocab_size):
#     if '(' not in vocab or ')' not in vocab:
#         return tf.ones((tf.shape(x)[0], tf.shape(x)[1], vocab_size), dtype=tf.bool)
    
#     open_idx = vocab['(']
#     close_idx = vocab[')']
    
#     parens = tf.where(tf.equal(x, open_idx), 1,
#                      tf.where(tf.equal(x, close_idx), -1, 0))
#     paren_counts = tf.cumsum(parens, axis=1, exclusive=True)
    
#     close_allowed = paren_counts > 0
#     close_mask = tf.equal(tf.range(vocab_size)[None, None, :], close_idx)
#     close_mask = tf.logical_and(close_allowed[..., None], close_mask)
    
#     open_mask = tf.equal(tf.range(vocab_size)[None, None, :], open_idx)
#     open_mask = tf.ones_like(open_mask)  # Always allow opening
    
#     return tf.logical_or(
#         tf.logical_and(close_mask, close_allowed[..., None]),
#         tf.logical_not(close_mask)
#     )
# def parentheses_rule(x, vocab, vocab_size):
#     if '(' not in vocab or ')' not in vocab:
#         return tf.ones((tf.shape(x)[0], tf.shape(x)[1], vocab_size), dtype=tf.bool)
...
#     # 提取token索引（假设x的最后一维是token embedding，这里需要取token ID）
#     # 注意：根据你的实际实现，这里可能需要用tf.argmax()或其他方式获取token ID
#     # 这里假设x是one-hot编码，直接取最大值的索引
#     token_ids = tf.argmax(x, axis=-1)  # 形状变为(32, 74)
    
#     # 跟踪括号计数（修正维度处理）
#     parens = tf.where(tf.equal(token_ids, open_idx), 1,
#                      tf.where(tf.equal(token_ids, close_idx), -1, 0))
#     paren_counts = tf.cumsum(parens, axis=1, exclusive=True) 
    
#     # 生成闭合括号mask（修正维度扩展方式）
#     close_allowed = paren_counts > 0  # 形状(32, 74)
#     close_mask = tf.equal(tf.range(vocab_size), close_idx)  # 形状(1000,)
    
#     # 将两个mask扩展为可广播形状
#     close_allowed = close_allowed[..., tf.newaxis]  # 形状(32, 74, 1)
#     close_mask = close_mask[tf.newaxis, tf.newaxis, :]  # 形状(1, 1, 1000)
    
#     # 组合最终mask（使用广播机制代替tf.where）
#     final_mask = tf.logical_or(
#         tf.logical_and(close_mask, close_allowed),
#         tf.equal(tf.range(vocab_size), open_idx)
#     )
    
#     return final_mask
def ring_rule(x, vocab, vocab_size):
    # 创建环数字映射表（1-9, %10-%99）
    ring_map = {}
    for token, idx in vocab.items():
        if len(token) == 1 and token.isdigit():
            ring_map[idx] = int(token)
        elif token.startswith('%') and token[1:].isdigit():
            ring_map[idx] = int(token[1:])
    
    if not ring_map:
        return tf.ones((tf.shape(x)[0], tf.shape(x)[1], vocab_size), dtype=tf.bool)
    
    max_ring = max(ring_map.values()) if ring_map else 0
    batch_size = tf.shape(x)[0]
    seq_len = tf.shape(x)[1]

    # 初始化环状态张量（batch, seq+1, max_ring+1）
    ring_state = tf.zeros([batch_size, seq_len+1, max_ring+1], dtype=tf.int32)
    
    # 构建环操作矩阵
    ring_ops = tf.zeros([batch_size, seq_len, max_ring+1], dtype=tf.int32)
    ...
    for t in range(seq_len):
        ...
        # 生成当前token的环操作（向量化处理）
        def get_ops(tensor):
            r = ring_map.get(tensor.numpy(), 0)
            return 1 if r > 0 else 0, r
        
        # 向量化处理获取环操作
        ops_info = tf.map_fn(
            lambda x: tf.py_function(get_ops, [x], [tf.int32, tf.int32]),
            tokens,
            fn_output_signature=[tf.TensorSpec([], tf.int32), tf.TensorSpec([], tf.int32)]
        )
        
        is_ring_token = ops_info[0]
        ring_numbers = ops_info[1]
        
        # 计算出现次数
        prev_counts = tf.reduce_sum(
            tf.cast(tf.equal(x[:, :t], tokens[:, None]), tf.int32),
            axis=1
        )
        
        # 确定操作类型（1: 开环，-1: 闭环）
        ops = tf.where(prev_counts % 2 == 0, 1, -1)
        ops = ops * tf.cast(is_ring_token, tf.int32)
        
        # 更新操作矩阵（向量化）
        batch_indices = tf.range(batch_size)[:, None]
        ring_indices = tf.cast(ring_numbers, tf.int32)[:, None]
        update_indices = tf.concat([batch_indices, t*tf.ones_like(batch_indices), ring_indices], axis=1)
        
        ring_ops = tf.tensor_scatter_nd_add(
            ring_ops,
            update_indices,
            ops
        )
    ...
    # 计算累积环状态
    cumulative_ops = tf.cumsum(ring_ops, axis=1)
    ring_state = tf.pad(cumulative_ops, [(0,0), (1,0), (0,0)])
    
    # 生成有效掩码
    valid_mask = tf.ones([batch_size, seq_len, vocab_size], dtype=tf.bool)
    
    for token_idx, r in ring_map.items():
        # 获取当前环状态
        current_state = ring_state[:, :-1, r]  # (batch, seq)
        
        # 开环条件：当前状态为偶数
        open_cond = (current_state % 2) == 0
        # 闭环条件：当前状态 > 0 且为奇数
        close_cond = (current_state > 0) & (current_state % 2 == 1)
        
        # 创建token掩码
        token_mask = tf.where(
            tf.equal(tf.range(vocab_size), token_idx),
            tf.logical_or(open_cond[..., None], close_cond[..., None]),
            True
        )
        
        valid_mask = tf.logical_and(valid_mask, token_mask)
...
def parentheses_rule(x, vocab, vocab_size):
    ...
    # 确保处理二维token索引（假设x是三维的，取最后一个维度的argmax）
    x_indices = tf.argmax(x, axis=-1) if len(x.shape) == 3 else x
    
    # 计算括号计数
    parens = tf.where(tf.equal(x_indices, open_idx), 1,
                     tf.where(tf.equal(x_indices, close_idx), -1, 0))
    paren_counts = tf.cumsum(parens, axis=1, exclusive=True)
    
    # 生成闭合括号mask
    close_allowed = paren_counts > 0  # 形状 (batch, seq)
    close_mask = tf.equal(tf.range(vocab_size), close_idx)  # 形状 (vocab_size,)
    close_mask = tf.logical_and(close_allowed[..., tf.newaxis], 
                               close_mask[tf.newaxis, tf.newaxis, :])  # 广播到 (batch, seq, vocab_size)
    
    # 开放括号mask（总是允许）
    open_mask = tf.equal(tf.range(vocab_size), open_idx)
    open_mask = tf.ones_like(open_mask, dtype=tf.bool)
    
    # 组合规则并确保输出形状正确
    final_mask = tf.where(close_mask, close_allowed[..., tf.newaxis], open_mask)
    return final_mask


def basic_atom_rule(x, vocab, vocab_size):
    # 示例原子约束：禁止连续两个原子符号
    atom_symbols = ['C', 'N', 'O']  # 扩展为完整列表
    atom_indices = [vocab[s] for s in atom_symbols if s in vocab]
    
    mask = tf.ones((tf.shape(x)[0], tf.shape(x)[1], vocab_size), dtype=tf.bool)
    
    for idx in atom_indices:
        prev_atom = tf.pad(x[:, :-1] == idx, [[0,0], [1,0]], constant_values=False)
        curr_mask = tf.logical_not(prev_atom[..., None] & (tf.range(vocab_size) == idx))
        mask = tf.logical_and(mask, curr_mask)
...
class SMILESDecoder(keras.Model):
    ...
    def __init__(self, hp,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num,vocab_size):
        ...
        # self.chemistry_layer = FullChemistryConstraint(vocab, atom_valence, bond_orders)
        self.constraint_layer = ChemicalConstraintLayer(
            vocab=vocab,
            rule_functions=[parentheses_rule,ring_rule]
        )
    ...
    def call(self, inputs, training=True):
        ...
        # 应用化学约束
        constrained_logits = self.constraint_layer([x, logits])
        # constrained_logits = self.constraint_layer(logits, x)
