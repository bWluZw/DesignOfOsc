...
from SMILES_vocab import get_token


# TODO 原子级TOKEN，和BPE算法，smiles的预处理
# 屏蔽所有RDKit的日志信息
...
def load_data(csv_file):
    ...
    for s, other_item in zip(smiles_list, normalized_other):
        ...
        if graph_data is not None:
            graph_data_list.append([graph_data[0],graph_data[1],graph_data[2],graph_data[3],s])
    ...
    dataset = tf.data.Dataset.from_tensor_slices((tf.convert_to_tensor(item1_list,dtype=tf.float32,name='node_features'),
                                                  ...
                                                  tf.convert_to_tensor(item5_list,dtype=tf.string,name='smiles'),
                                                  ))
    ...
    return dataset, node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num
...
class GAN(keras.Model):
    ...
    def _compute_loss(
        self, z_log_var, z_mean, real_data, fake_data
    ):
        real_nf, real_ef,real_es, real_ed, real_prop = real_data[0],real_data[1],real_data[2],real_data[3],real_data[4]
        
        fake_nf, fake_ef,fake_es, fake_ed, fake_prop = fake_data[0],fake_data[1],fake_data[2],fake_data[3],fake_data[4]
        
        # 假设填充部分的节点特征全为0，掩码为1表示有效节点
        node_mask = tf.reduce_sum(real_nf, axis=-1) > 0  # (batch_size, max_nodes)
        node_mask = tf.cast(node_mask, tf.float32)

        # 计算节点特征损失 (Mean Squared Error for regression)
        nf_loss = tf.reduce_mean(
            tf.reduce_sum(keras.losses.categorical_crossentropy(real_nf, fake_nf)* node_mask, axis=1)
            # tf.reduce_sum(keras.losses.mean_squared_error(real_nf, fake_nf), axis=1)
        ) / tf.reduce_sum(node_mask)
        # nf_loss = tf.reduce_mean(
        #     tf.reduce_sum(keras.losses.categorical_crossentropy(real_nf, fake_nf), axis=1)
        # ) 

        # 同理处理边特征掩码
        edge_mask = tf.reduce_sum(real_ef, axis=-1) > 0  # (batch_size, max_edges)
        edge_mask = tf.cast(edge_mask, tf.float32)
        
        edge_mask_reduce_sum_res = tf.reduce_sum(edge_mask)
        
        # 计算边特征损失 (Mean Squared Error for regression)
        ef_loss = tf.reduce_mean(
            tf.reduce_sum(keras.losses.categorical_crossentropy(real_ef, fake_ef)*edge_mask, axis=1)
            # tf.reduce_sum(keras.losses.mean_squared_error(real_ef, fake_ef), axis=1)
        )/ edge_mask_reduce_sum_res
        # ef_loss = tf.reduce_mean(
        #     tf.reduce_sum(keras.losses.categorical_crossentropy(real_ef, fake_ef), axis=1)
        # )
        

        # # 计算节点标签损失 (Mean Squared Error for regression)
        # es_part_loss_fn = keras.losses.CategoricalCrossentropy(from_logits=False)
        # es_loss = es_part_loss_fn(real_es, fake_es)
        # # es_loss = tf.reduce_mean(
        # #     tf.reduce_sum(es_part_loss, axis=-1)
        # # )


        # # 计算边标签损失 (Mean Squared Error for regression)
        # ed_loss = es_part_loss_fn(real_ed, fake_ed)
        
                # 使用稀疏分类交叉熵（适用于整数索引）
        es_loss = keras.losses.SparseCategoricalCrossentropy(from_logits=False)(real_es, fake_es)
        es_loss = tf.reduce_sum(es_loss * edge_mask)
        es_loss = es_loss / edge_mask_reduce_sum_res

        ed_loss = keras.losses.SparseCategoricalCrossentropy(from_logits=False)(real_ed, fake_ed)
        ed_loss = tf.reduce_sum(ed_loss * edge_mask)
        ed_loss = ed_loss / edge_mask_reduce_sum_res


        # 计算gap值损失 (Mean Squared Error for regression)
        prop_loss = tf.reduce_mean(
            tf.reduce_sum(keras.losses.mean_squared_error(real_prop, fake_prop), axis=-1)
        )
        kl_loss = -0.5 * ops.sum(1 + z_log_var - z_mean**2 - ops.minimum(ops.exp(z_log_var), 1e6), 1)
        kl_loss = ops.mean(kl_loss)
        res = kl_loss  + nf_loss + ef_loss + es_loss + ed_loss + prop_loss
        return res,kl_loss , nf_loss ,ef_loss,es_loss,ed_loss,prop_loss
    ...
    def _gradient_penalty(self, real_data, fake_data):
        ...
        nf_interp = (real_nf * alpha) + (1.0 - alpha) * fake_nf
        
        alpha = ops.reshape(alpha, (self.batch_size, 1, 1))
        ef_interp = (real_ef * alpha) + (1.0 - alpha) * fake_ef
        
        
        real_es_float = tf.cast(real_es,tf.float32)
        fake_es_float = tf.cast(fake_es,tf.float32)
        alpha = ops.reshape(alpha, (self.batch_size, 1))
        es_interp = (real_es_float * alpha) + (1.0 - alpha) * fake_es_float
        
        real_ed_float = tf.cast(real_ed,tf.float32)
        fake_ed_float = tf.cast(fake_ed,tf.float32)
        alpha = ops.reshape(alpha, (self.batch_size, 1))
        ed_interp = (real_ed_float * alpha) + (1.0 - alpha) * fake_ed_float
        
        alpha = ops.reshape(alpha, (self.batch_size, 1))
        prop_interp = (real_prop * alpha) + (1.0 - alpha) * fake_prop


        # Compute the logits of interpolated graphs
        with tf.GradientTape() as tape:
            tape.watch(nf_interp)
            tape.watch(ef_interp)
            tape.watch(es_interp)
            tape.watch(ed_interp)
            tape.watch(prop_interp)
            _, _, logits, _ = self(
                [nf_interp, ef_interp,es_interp,ed_interp,prop_interp], training=True
            )

        # Compute the gradients with respect to the interpolated graphs
        grads = tape.gradient(logits, [nf_interp, ef_interp,es_interp,ed_interp,prop_interp])
        # Compute the gradient penalty
        grads_nf_penalty = (1 - ops.norm(grads[0], axis=1)) ** 2
        grads_ef_penalty = (1 - ops.norm(grads[1], axis=2)) ** 2
        return ops.mean(
            ops.mean(grads_nf_penalty, axis=(-1))
            + ops.mean(grads_ef_penalty, axis=(-1))
        )
    def inference(self, batch_size):
        
        z = keras.random.normal(
            shape=(batch_size, self.max_atom_type_num), seed=self.seed_generator
        )
        ...
        adjacency = ops.one_hot(adjacency, num_classes=self.max_bond_type_num, axis=1)
        # Remove potential self-loops from adjacency
        adjacency = adjacency * (1.0 - ops.eye(self.max_atom_num, dtype="float32")[None, None])
        # obtain one-hot encoded feature tensor
        features = ops.argmax(reconstruction_features, axis=2)
        features = ops.one_hot(features, num_classes=self.max_atom_type_num, axis=2)
        return [
            graph_to_molecule([adjacency[i].numpy(), features[i].numpy()])
            for i in range(batch_size)
        ]
    
    def physical_score(self,data,multiple=10):
        """
        简单计算是否符合物理规律
        ...
        for nf,ef,es,ed in zip(data[0],data[1],data[2],data[3]):
            es = tf.argmax(es, axis=1,output_type=tf.int32)
            ed = tf.argmax(ed, axis=1,output_type=tf.int32)
            smiles,mol = graph_to_smiles((nf,ef,es,ed))
    ...
    def train_step(self, data):
        ...
        real_data = (data[0],data[1],data[2],data[3],data[4])
        
        self.batch_size = tf.shape(real_data[0])[0]
        with tf.GradientTape() as tape:
            z_mean, z_log_var, fake_data = self(real_data, training=True)
        ...
        physical_score = self.physical_score(fake_data)
...
class TransformerDecoderBlock(layers.Layer):
    def __init__(self, d_model, num_heads, dff, dropout=0.1):
        super().__init__()
        self.mha1 = tensorflow_addons.layers.MultiHeadAttention(num_heads=num_heads, key_dim=d_model//num_heads)
        self.mha2 = tensorflow_addons.layers.MultiHeadAttention(num_heads=num_heads, key_dim=d_model//num_heads)
...
class SMILESDecoder(keras.Model):
    ...
    def __init__(self, hp,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num):
        ...
        # SMILES字符集大小
        self.vocab_size = max_smiles_char_type_num
        # 注意力头数
        self.num_heads = 8
        # 前馈网络维度
        self.dff = 2048
...
class HyperGAN(kt.HyperModel):
    ...
    def __init__(self,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num):
    ...
    def make_decoder_model(self, hp):
        ...
        decoder = SMILESDecoder(hp,self.config,self.nf_shape, self.ef_shape,self.max_atom_num,self.max_atom_type_num,self.max_bond_type_num,self.edge_len_num,self.max_smiles_len,self.max_smiles_char_type_num)
...
def main():
    ...
    dataset, node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num = load_data(csv_file)
    ...
    tuner = kt.BayesianOptimization(
        hypermodel=HyperGAN(search_space,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num),
