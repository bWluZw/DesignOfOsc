...
def load_data(csv_file):
    ...
    vocab_temp = token_obj.tokenizer.get_vocab()
    VOCAB_DIC={}
    for key, value in vocab_temp.items():
        VOCAB_DIC[value] = key
    # Prepare graph data
    max_nodes = 0
    max_bond_num = 0
...
class TransformerDecoderBlock(layers.Layer):
    def __init__(self, d_model, num_heads, dff, dropout=0.1):
        ...
        # ])
        self.ffn = keras.Sequential([
            layers.Dense(dff, activation='gelu', kernel_regularizer='l2'),
            layers.Dense(d_model, kernel_regularizer='l2')
        ])
    ...
    def call(self, x, z, training, look_ahead_mask):
        ...
        # 交叉注意力：z 作为 key/value
        z_proj = tf.expand_dims(keras.layers.Dense(self.d_model)(z), 1)
        z_proj = tf.tile(z_proj, [1, tf.shape(x)[1], 1])
        attn2 = self.mha2(out1, z_proj, z_proj)  # query=out1, key/value=z_proj
        ...
        # return out3
class SMILESConstraint:
    def __init__(self):
        self.open_rings = set()  # 未闭合的环编号
        self.branch_stack = []   # 分支括号的嵌套层级
        self.valence_tracker = {} # 原子价跟踪（示例）
    ...
    def update_state(self, token):
        ...
        elif token.isdigit():  # 环编号处理
            if int(token) in self.open_rings:
                self.open_rings.remove(int(token))
            else:
                self.open_rings.add(int(token))

    def get_valid_tokens(self, current_seq):
        # 返回当前允许的Token列表（需扩展完整规则）
        valid_tokens = set()
        
        # 示例规则1: 括号必须成对
        if '(' in current_seq and current_seq[-1] != ')':
            valid_tokens.add(')')
        
        # 示例规则2: 环编号1-9需配对
        used_rings = set([c for c in current_seq if c.isdigit()])
        valid_rings = set(str(i) for i in range(1,10) if i not in used_rings)
        valid_tokens.update(valid_rings)
        
        # 示例规则3: 原子价约束（需自定义完整实现）
        last_atom = self._get_last_atom(current_seq)
        if last_atom in ['C', 'N']:
            valid_tokens.update(['=', '#'])  # 假设允许双键、三键
...
class SMILESDecoder(keras.Model):
    ...
    def generate_validity_mask(self, current_seq):
        # """生成动态有效性遮罩"""
        # mask = tf.ones([len(self.vocab)], dtype=tf.float32) * -1e9
        # valid_tokens = self.constraint.get_valid_tokens(current_seq)
        
        # # 将合法Token的位置设为0（允许选择）
        # for idx, token in enumerate(self.vocab):
        #     if token in valid_tokens or token == '<pad>':  # 始终允许pad（根据实际情况调整）
        ...
        # 设置允许的token
        for idx, token in enumerate(self.vocab):
            if token in valid_tokens or token == '<eos>':  # 始终允许结束符
                mask = tf.tensor_scatter_nd_update(
                    mask,
                    [[idx]],
                    [0.0]  # 将允许的token位置设为0
    ...
    def decode_step(self, inputs, states):
        """单步解码函数"""
        x, z = inputs
        output = self([x, z], training=False)
        logits = output[:, -1, :]
        
        # 应用动态遮罩
        current_seq = self._tokens_to_string(x[0])  # 需实现tokens到字符串的转换
        validity_mask = self.generate_validity_mask(current_seq)
        masked_logits = logits + validity_mask
        
        return masked_logits, states
    
    def constrained_generate(self, z, max_length=100):
        start_token = self.vocab.index('<sos>')  # 使用开始符
        current_tokens = tf.constant([[start_token]], dtype=tf.int32)  # (1,1)
        self.constraint = SMILESConstraint()  # 重置约束状态
        
        for _ in range(max_length):
            # 获取当前序列状态
            current_seq_str = self._get_current_seq(current_tokens)[0]
            
            # 生成下一步logits
            logits = self([current_tokens, z], training=False)
            
            # 应用动态遮罩
            valid_mask = self.generate_validity_mask(current_seq_str)
            masked_logits = logits[:, -1, :] + valid_mask  # 仅处理最后一个token
            
            # 选择token（这里使用贪心采样）
            next_token = tf.argmax(masked_logits, axis=-1, output_type=tf.int32)
            
            # 更新序列
            current_tokens = tf.concat([current_tokens, [next_token]], axis=-1)
            
            # 遇到结束符停止
            if next_token.numpy()[0] == self.vocab.index('<eos>'):
                break
                
        # 最终转换
        return self._get_current_seq(current_tokens)[0]
    
    def _get_current_seq(self, token_indices):
        """
        将token索引转换为SMILES字符串
        输入形状: (batch_size, seq_len)
        返回: 当前批次所有序列的字符串列表
        """
        batch_sequences = []
        for batch in token_indices.numpy():
            current_seq = []
            for idx in batch:
            ...
            batch_sequences.append(''.join(filtered_seq))
    ...
    def call(self, inputs, training=True):
        ...
        if not training:  # 推理时应用动态约束
            current_seq = self._get_current_seq(x)  # 需实现序列获取方法
            validity_mask = self.generate_validity_mask(current_seq)
