...
class SimplifiedRingLayer(layers.Layer):
    """简化版环规则层（效果保留80%，实现复杂度降低60%）"""
    def __init__(self, ring_tokens):
        super().__init__()
        self.ring_tokens = tf.constant(ring_tokens)
    ...
    def call(self, inputs):
        ...
        # 关键规则1：禁止连续两个环编号
        last_token = seq[:, -1:]
        is_last_ring = tf.reduce_any(tf.equal(last_token, self.ring_tokens), axis=-1)
        mask_cond1 = tf.where(is_last_ring, 0.0, 1.0)[..., tf.newaxis]
        
        # 关键规则2：环编号必须成对出现
        ring_counts = tf.math.bincount(seq, weights=tf.ones_like(seq, tf.float32))
        expected_closing = ring_counts % 2
        mask_cond2 = tf.gather(expected_closing, self.ring_tokens)
        
        # 合并掩码
        mask = mask_cond1 * mask_cond2
        return logits * mask + (1 - mask) * -1e9

    
class FastValenceLayer(layers.Layer):
    """高效价态校验层（结合RDKit实时解析）"""
    def __init__(self, atom_valence, bond_token_indices):
        super().__init__()
        self.atom_valence = atom_valence  # 原子价态规则，如{'C':4}
        self.bond_token_indices = bond_token_indices  # 需校验的键类型token索引，如[1,2,3]

    def _smiles_to_mol(self, smiles_str):
        """将SMILES转换为分子对象（增加异常处理）"""
        try:
    ...
    def _bond_count_for_atom(self, mol, atom_idx):
        """计算指定原子的总键级"""
    ...
    def _get_last_atom_valence(self, smiles_tensor):
        """获取最后一个原子的剩余价态（增强鲁棒性）"""
        try:
            # 转换张量到字符串
            smiles_str = smiles_tensor.numpy().decode('utf-8')
            mol = self._smiles_to_mol(smiles_str)
            
            if not mol or mol.GetNumAtoms() == 0:
                return 4.0  # 默认返回碳的价态
            
            last_atom_idx = mol.GetNumAtoms() - 1
            atom = mol.GetAtomWithIdx(last_atom_idx)
            used_valence = self._bond_count_for_atom(mol, last_atom_idx)
            max_valence = self.atom_valence.get(atom.GetSymbol(), 4)
            return max_valence - used_valence
        except:
            return 4.0  # 异常时返回默认价态

    def call(self, inputs):
        logits, current_smiles = inputs
        batch_size = tf.shape(logits)[0]
        vocab_size = tf.shape(logits)[-1]

        valences = tf.map_fn(
            lambda s: tf.py_function(
                func=self._get_last_atom_valence,
                inp=[s],
                Tout=tf.float32
            ),
            current_smiles,
        ...
        mask = tf.ones((batch_size, 1, vocab_size), dtype=tf.float32)
        indices = []
        updates = []

        for i in range(batch_size):
            current_valence = valences[i]
            for bond_idx in self.bond_token_indices:
                allow = 1.0 if current_valence >= bond_idx else 0.0
                indices.append([i, 0, bond_idx])
                updates.append(allow)

        if indices:
            mask = tf.tensor_scatter_nd_update(
                mask,
                tf.constant(indices, dtype=tf.int32),
                tf.constant(updates, dtype=tf.float32)
            )
...
class SMILESDecoder(keras.Model):
    ...
    def __init__(self, hp,config,node_features_shape,edge_features_shape,max_atom_num,max_atom_type_num,max_bond_type_num,edge_len_num,max_smiles_len,max_smiles_char_type_num,vocab_size):
        ...
        # 添加规则层
        self.valence_layer = FastValenceLayer(atom_valence=atom_valence,bond_token_indices=bond_tokens)
