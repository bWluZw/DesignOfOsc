...
class Generator(nn.Module):
    def __init__(self, noise_dim, fingerprint_dim, feature_dim, cond_dim, hidden_dim=128, dropout=0.3):
        """
        条件生成器模型 (cGAN)
        Args:
    ...
    def forward(self, noise, condition):
        ...
        Args:
            ...
            condition (torch.Tensor): 条件向量 (batch_size, cond_dim)
        Returns:
        ...
        hidden = self.input_layer(input_data)
...
def train_and_evaluate(generator, discriminator, g_optimizer, d_optimizer, 
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            generated_data = generator(noise,real_features)
...
# 将 PyTorch Tensor 转换为 RDKit ExplicitBitVect
def tensor_to_explicit_bitvect(tensor):
    # 将 Tensor 转换为二进制列表
    bit_list = tensor.tolist()
    # 使用 RDKit 创建 ExplicitBitVect
    bit_vect = DataStructs.ExplicitBitVect(len(bit_list))
...
#     "cond_dim": 3,
#     "hidden_dim": 64,
#     "dropout": 0.4596295401829442,
#     "batch_size": 32,
#     "num_epochs": 10,
#     "learning_rate": 0.0004723332516616578,
#     "g_lr": 0.00007516901232931335,
#     "d_lr": 0.0001428127726502546
# }
    # 参数设置
    csv_path = "D:\Project\ThesisProject\AutoML\data\SMILES_donors_and_NFAs.csv" 
    save_path = "D:\Project\ThesisProject\AutoML\Project2\GANProject\generator.pyh" 
    ...
    # 初始化模型
    # (self, noise_dim, fingerprint_dim, feature_dim, cond_dim, hidden_dim=128, dropout=0.3):
    generator = Generator(noise_dim, fingerprint_dim, feature_dim, cond_dim, hidden_dim, dropout)
    ...
    with torch.no_grad():
        ...
        condition = features_res[:10].to(device)
        ...
        valid_smiles = []
        # 判断生成的指纹是否可以还原为有效的SMILES
        for item in fingerprints:
            try:
                mol = Chem.RDKFingerprintToMol(item)
                smiles = Chem.MolToSmiles(mol)
                valid_smiles.append(1)
            except:
                valid_smiles.append(0)
        # 计算生成特性与真实特性之间的平均差异，反映相似性
        diff = torch.abs(features - condition).mean(dim=0)
        similarity_score = 1 - diff.mean().item()
        
        
        # 通过Tanimoto相似性计算生成分子的平均新颖性
        scores = []
        for gen_fp in fingerprints:
            # 计算每个生成分子与训练集中所有分子之间的相似性
            max_similarity = 0
            for real_fp in fingerprint_res:
                similarity = DataStructs.TanimotoSimilarity(tensor_to_explicit_bitvect(gen_fp), tensor_to_explicit_bitvect(real_fp))
                # similarity = TanimotoSimilarity(gen_fp, real_fp)
                max_similarity = max(max_similarity, similarity)
            # 1减去最大相似性，得到新颖性得分
            novelty_score = 1 - max_similarity
            scores.append(novelty_score)
        
        validity_rate = sum(scores) / len(scores)
        # 综合得分
        composite_score = 0.4 * validity_rate + 0.4 * similarity_score + 0.2 * novelty_score

            # 向 NNI 传递结果
        nni.report_final_result(composite_score)
