...
"""
def train_and_cross_validate(model, x, y, config, params, n_splits=5):
    """
    使用交叉验证训练和评估模型
    :param model_class: 传入模型类（未初始化）
    :param x: 输入特征
    :param y: 标签
    :param config: 配置字典
    :param params: 超参数字典
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(x, y)):
        ...
        # # 初始化模型
        # model = model_class(
        #     input_dim=config.get('input_dim', 2048 + 2),
...
def main():
    try:
        ...
        # 训练和评估模型
        # train_and_cross_validate(model_class, x, y, config, params, n_splits=5):
        accuracy = train_and_cross_validate(model, x, y, config, params)
