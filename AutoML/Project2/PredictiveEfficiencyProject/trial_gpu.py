...
def load_data(csv_file, batch_size=32):
    ...
    # 遍历smiles、homo、lumo和pce的元素
    for smile, h, l, p in zip(smiles, homo, lumo, pce):
        # 将每个smile、h、l和p转换为分子图
        graph = smiles_to_graph(smile, h, l, p)
        # 将生成的分子图添加到列表中
        if graph is None:
            continue
...
class GATModel(nn.Module):
    def __init__(self, in_channels, hidden_channels=64, out_channels=1, heads=16, dropout=0.01):
        ...
        self.conv2 = GATConv(hidden_channels * heads, hidden_channels * heads, heads=heads)
        ...
        self.conv3 = GATConv(hidden_channels * heads, hidden_channels * heads, heads=heads)
...
def main():
    ...
    # 构建模型
    model = GATModel(in_channels=4, hidden_channels=params["hidden_dim"], out_channels=1,heads=params["heads"], dropout=params["dropout"])
