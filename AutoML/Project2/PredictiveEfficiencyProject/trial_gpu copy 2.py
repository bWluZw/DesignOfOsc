...
def load_data(csv_file, batch_size=32):
    ...
    #     test1,test2 = data  # 将数据迁移到GPU

    #     print(f"After train_loader: {test1.shape}")
    # print(f"After test_loader: {test2}")
    return train_loader, test_loader, pce_scaler, homo_scaler, lumo_scaler


class GATModel(nn.Module):
    def __init__(self, in_channels=3, hidden_channels=64, out_channels=1, heads=16, dropout=0.2):
        ...
        # self.fc_out = nn.Linear(out_channels, 1)  # out_channels * heads 是你最后输出的维度
    ...
    def forward(self, data):
        ...
        # 聚合所有节点的特征，得到图级别的预测值
        # x = torch.mean(x, dim=1, keepdim=False)  # 聚合所有节点特征
