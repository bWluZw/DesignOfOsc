...
# import torch.nn as nn 
# import torch.nn.functional as F 
# from nni.nas.pytorch.fixed 
# import apply_fixed_architecture 
# from nni.nas.hub.pytorch import DartsMutator, DartsTrainer
# import nni.nas.hub.pytorch as nasp
# # 定义训练函数
# def train_model(config):
#     model = PDARTSModel(channels=16)
#     mutator = PDARTSMutator(model)
#     criterion = nn.CrossEntropyLoss()
#     optimizer = SGD(model.parameters(), lr=config['lr'], momentum=0.9, weight_decay=3e-4)
...
#         mutator=mutator,
#         loss=criterion,
#         metrics=lambda output, target: (output.argmax(dim=1) == target).float().mean(),
#         optimizer=optimizer,
#         num_epochs=10,
#         dataset=trainloader,
#         batch_size=64,
...
#     valid_acc = trainer.evaluate(valid_loader=testloader)
