...
class GAN(keras.Model):
    ...
    def train_step(self, real_data):
        ...
        # gen_score = self.score_loss(gen_loss,self.gen_loss_list[len(self.gen_loss_list)-1])
        # dics_score = self.score_loss(disc_loss,self.disc_loss_list[len(self.disc_loss_list)-1])
        # 返回每个epoch的损失值
        print(f'步：')
        return {
            'd_loss_score': gen_loss,
            'g_loss_score': disc_loss,
        }
...
class HyperGAN(kt.HyperModel):
    ...
    def fit(self, hp, model, x, callbacks=None,**kwargs):
        ...
        all_data = x.batch(64)
        @tf.function
        def run_train_step(real_data):
            
            # gen_data_list = tf.split(real_data[0], num_or_size_splits=64, axis=0)
            # disc_data_list = tf.split(real_data[1], num_or_size_splits=64, axis=0)
            gen_data_list = tf.unstack(real_data[0],  axis=0)
            disc_data_list = tf.unstack(real_data[1],  axis=0)
            for gen_data,disc_data in zip(gen_data_list,disc_data_list):
                # item1 = item[0]
                # item2 = item[1]
                # 随机生成噪声向量,从标准正态分布中生成一个随机向量，表示潜在空间的样本。
                noise1 = tf.random.normal(shape=self.input_dim1_shape)  # 噪声维度
                ...
                with tf.GradientTape() as tape_d:
                    # 判别器评估真实分子指纹和生成的分子指纹
                    real_output = self.discriminator([gen_data,disc_data], training=True)
                    fake_output = self.discriminator(generated_data, training=True)
                    real_loss = self.loss_fn(tf.ones_like(real_output), real_output)
                    fake_loss = self.loss_fn(tf.zeros_like(fake_output), fake_output)
                    d_loss = real_loss + fake_loss
        ...
        for epoch in range(10):
            ...
            for callback in callbacks:
                callback.on_epoch_end(epoch, logs={"gen_loss": gen_loss_float,"disc_loss":disc_loss_float})
            self.gen_loss_tracker.reset_state()
            self.disc_loss_tracker.reset_state()
            print(f'gen_loss:{gen_loss_float},disc_loss:{disc_loss_float}')
            best_gen_loss = min(best_gen_loss, gen_loss_float)
...
def main():
    ...
    tuner = kt.BayesianOptimization(
        ...
        objective=kt.Objective("g_loss_score", "max"),
